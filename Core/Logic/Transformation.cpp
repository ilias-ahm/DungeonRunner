...
std::pair<float, float> Transformation::_toPixel(std::shared_ptr<sf::RenderWindow> gWindow, float x, float y)
{
        ...
        double newY;
        if (y > 0)
                newY = windowSize.second - (rPosY * lenY) - ((int)y + wSize.second / 2.0) * windowSize.second;
        else
                newY = windowSize.second + (rPosY * lenY) - ((int)y + wSize.second / 2.0) * windowSize.second;
        return std::pair<float, float>(newX, newY);
}

Transformation::Transformation() { Transformation::wSize = std::pair<int, int>(4, 14); }
...
std::pair<int, int> Transformation::getWSize() { return getInstance()._getWSize(); }
...
std::pair<int, int> Transformation::_getWSize() { return wSize; }
