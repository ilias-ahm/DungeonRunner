...
DungeonRunner::Highscores::Highscores(const std::string& fileName) : fileName(fileName) { loadHighscores(fileName); }
...
void DungeonRunner::Highscores::writeToFile()
{
        std::ofstream wFile(fileName,
                            std::ofstream::out |
                                std::ofstream::trunc); // Flushes file and re-inserts every highscore into file
