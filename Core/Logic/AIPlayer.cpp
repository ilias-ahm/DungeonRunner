...
void DungeonRunner::AIPlayer::run() {
    ...
    if(aiSpeed >0.6) aiSpeed = 0.6; // max speed for ai
    if(aiSpeed <0.2) aiSpeed = 0.2; // min speed for ai
    avoidObstacles();

    //This piece of code makes sure that the ai never get stuck on something, they will move from left to right until they get unstuck
...
void DungeonRunner::AIPlayer::avoidObstacles() {
    ...
    for(auto &entity:gameEntities){
        if(entity->getType() == "Door" or entity->getType() == "Sword"){
            ...
            if(dY > 0 and dY < 0.6 and dX < entity->getESize().first/2.0 + eSize.first/2.0) { // If its in the same lane and y difference < 0.6 (in pixels that would be 0.6*windowsize.y)
                if(isDodging and !isStuck) return; // If ai is already dodging obstacle but isnt stuck, dont interrupt
                if(entity->getType() == "Sword" and Random::generateRandomChance() <0.2  and !stuckResolving) { // if ai is not stuck, there is a 20% chance every loop to dodge a sword
                ...
                if(entity->getType() == "Door") {
                    ...
                    if (Random::generateRandomChance() < 0.0025) { // if ai is stuck, it will still try to open doors to avoid being stuck permanently
...
void DungeonRunner::AIPlayer::dodgeObstacle(std::shared_ptr<DungeonRunner::Entity> obstacle) {
    ...
    if(ePosition.first <-0.5) dodgeState = 2; // if ai is on left side of map, dodge right
    if(ePosition.first >0.5) dodgeState = 1; // if ai is on right side of map, dodge left
    isDodging = true;
}
