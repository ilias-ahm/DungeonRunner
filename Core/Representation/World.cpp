...
DungeonRunner::World::World(std::shared_ptr<sf::RenderWindow> gWindow, int x, int y)
{
        ...
        initWorld();
        // Create finishline
        std::pair<float, float> entitySize =
            Transformation::toCoords(gWindow, gameWindow->getSize().x / 2.0, gameWindow->getSize().y / 8.0);
...
{ // Generates world map, first code i wrote so may be a little unreadable
        for (int board = 0; board != worldSize.second; board++) {
                std::shared_ptr<sf::RectangleShape> Wall1 = std::make_shared<sf::RectangleShape>(
                    sf::Vector2f(gameWindow->getSize().x / 8.0, gameWindow->getSize().y));
                std::shared_ptr<sf::RectangleShape> Wall2 = std::make_shared<sf::RectangleShape>(
                    sf::Vector2f(gameWindow->getSize().x / 8.0, gameWindow->getSize().y));
                ...
                Wall2->setPosition(gameWindow->getSize().x / 2.0 + gameWindow->getSize().x / 4.0,
                                   (float)board * (-(float)gameWindow->getSize().y));
                ...
                for (int lane = 0; lane != worldSize.first; lane++) {
                        ...
                        while (currentRow != 8) {
                                ...
                                if (rand < 0.030 and board > 0 and board != worldSize.second - 1) {
                                        std::shared_ptr<sf::RectangleShape> dRec =
                                            std::make_shared<sf::RectangleShape>(sf::Vector2f(
                                                gameWindow->getSize().x / 10.0, gameWindow->getSize().y / 10.0));
                                        std::shared_ptr<DungeonRunnerSFML::DoorSFML> dObs =
                                            AbstractFactory::createDoor(dRec, gameWindow, obstacleTextures);
                                        dRec->setPosition(
                                            (float)((lane + 2) * gameWindow->getSize().x / 8) + dRec->getSize().x / 2.0,
                                            (float)(currentRow * gameWindow->getSize().y / 8) + dRec->getSize().y -
                                                (float)(board * gameWindow->getSize().y));
                                        dObs->setEPosition(Transformation::toCoords(gameWindow, dRec->getPosition().x,
                                                                                    dRec->getPosition().y));
                                ...
                                for (int i = 0; i != 4; i++) {
                                        ...
                                        tile->setSize(sf::Vector2f(gameWindow->getSize().x / 16.0,
                                                                   gameWindow->getSize().y / 16.0));
                                        ...
                                        case 0:
                                                ...
                                                break;
                                        case 1:
                                                tile->setPosition((float)(lane + 2) * (float)gameWindow->getSize().x /
                                                                          8 +
...
std::shared_ptr<sf::Texture> DungeonRunner::World::getRandomFloorTexture()
{
        ...
        if (0 <= random and random < 50)
                random = 1;
        else if (50 <= random and random < 53)
                random = 2;
        else if (53 <= random and random < 73)
                random = 3;
        else if (73 <= random and random < 90)
                random = 5;
        else {
