...
DungeonRunner::Game::Game(const std::shared_ptr<sf::RenderWindow>& gameWindow) : gameWindow(gameWindow)
{
        ...
        gameView = sf::View(sf::Vector2f(gameWindow->getSize().x / 2.0, 0),
        ...
        gameWorld->update();
        gameEntities.insert(gameEntities.end(), gameWorld->getWorldEntities().begin(),
                            gameWorld->getWorldEntities().end());
        ...
        std::shared_ptr<Entity> Wall1 =
            AbstractFactory::createCollider(std::pair<float, float>(-1.25, -7), std::pair<float, float>(0.5, 14));
        std::shared_ptr<Entity> Wall2 =
            AbstractFactory::createCollider(std::pair<float, float>(1.25, -7), std::pair<float, float>(0.5, 14));
...
void DungeonRunner::Game::update(double dTime)
{
        ...
        if (gamePlayer->getPos().y < mapCoords.second + gameWindow->getSize().y / 2.0 - gameWindow->getSize().y / 4.0)
                gameView.setCenter(gameWindow->getSize().x / 2.0,
                                   mapCoords.second); // set static view untill player progresses
        else
                gameView.setCenter(gameWindow->getSize().x / 2.0,
                                   gamePlayer->getPos().y -
                                       gameWindow->getSize().y / 4.0); // update view position to player position
        updateViewColliders();

        for (auto& entity : gameEntities) {
                if (entity == gamePlayer or entity->getType() == "AI")
                        continue;
                entity->update();
                entity->display();
        }
...
{
        std::shared_ptr<sf::RectangleShape> player = std::make_shared<sf::RectangleShape>(
            sf::Vector2f(gameWindow->getSize().x / 11.0, gameWindow->getSize().y / 11.0));
...
void DungeonRunner::Game::updateViewColliders()
{
        for (auto& collider : viewColliders) {
                ...
                if (collider->getEPosition().first != 0) {
                        newPos.second =
                            Transformation::toCoords(gameWindow, gameView.getCenter().x, gameView.getCenter().y)
                                .second -
                            collider->getESize().second / 2.0;
...
{
        if (finished)
                return; // doesnt check for collision when game ends
        for (auto& collider : gameEntities) {
                float push = 1;
                if (collider == entity)
                        continue;
                if (entity->getType() == "AI" and collider->getType() == "Player")
                        push = 0.5; // AI and player have the same pushing force
                if (collider->getType() == "AI" and entity->getType() == "Player")
                        push = 0.5; // AI and player have the same pushing force
                if (collider->getType() == "AI" and entity->getType() == "AI")
                        push = 0.5; // AI and AI have the same pushing force
                if (isColliding(collider, entity, push)) {
                        if (collider->getType() == "Sword" and
                            (entity->getType() == "Player" or entity->getType() == "AI")) {
                                collider->setEPosition(std::pair<float, float>(
                                    -3, 9)); // teleport sword to no mans land when it hits ai or player
                                entity->action();
                        }
                        if (collider->getType() == "Finish" and
                            (entity->getType() == "Player" or
                             entity->getType() ==
                                 "AI")) { // checks who finished in which place to give score accordingly
                                if (finishedPlayers == 0)
                                        entity->notifyObservers(DungeonRunner::Observer::finishedFirst);
                                if (finishedPlayers == 1)
                                        entity->notifyObservers(DungeonRunner::Observer::finishedSecond);
                                if (finishedPlayers == 2)
                                        entity->notifyObservers(DungeonRunner::Observer::finishedThird);
                                if (finishedPlayers == 3)
                                        entity->notifyObservers(DungeonRunner::Observer::finishedLast);
                                finishedPlayers++;
                                if (entity->getType() == "Player") {
                                        finished = true; // game ends when player reaches finishline
                                        gameScores->addScore(entity->getSubjectObservers()[0]->getObserverName(),
                                                             entity->getSubjectObservers()[0]->getObserverData());
                                        gameScores->writeToFile();
                                        std::sort(gameEntities.begin(), gameEntities.end(),
                                                  [](std::shared_ptr<DungeonRunner::Entity> first,
                                                     std::shared_ptr<DungeonRunner::Entity> second) {
                                                          return first->getEPosition().second <
                                                                 second->getEPosition().second;
                                                  }); // sorts entities from smallest y-value to highest
                                        for (auto& entity : gameEntities) { // places the rest of the hikers according
                                                                            // to position at time that player finishes
                                                if (entity->getType() == "AI" and entity->getEPosition().second < 7) {
                                                        if (finishedPlayers == 1)
                                                                entity->notifyObservers(
                                                                    DungeonRunner::Observer::finishedSecond);
                                                        if (finishedPlayers == 2)
                                                                entity->notifyObservers(
                                                                    DungeonRunner::Observer::finishedThird);
                                                        if (finishedPlayers == 3)
                                                                entity->notifyObservers(
                                                                    DungeonRunner::Observer::finishedLast);
                                                        finishedPlayers++;
                                                }
                                        }
                                }
...
void DungeonRunner::Game::managePlayerMovement(double dTime)
{
        if (sf::Keyboard::isKeyPressed(SPEEDUP)) {
                ...
                if (gamePlayer->getPlayerSpeed() * 1.01 > 0.5)
                        gamePlayer->setPlayerSpeed(0.5);
        }
        if (sf::Keyboard::isKeyPressed(SLOWDOWN)) {
                gamePlayer->setPlayerSpeed(gamePlayer->getPlayerSpeed() / 1.01); // slowdown
                if (gamePlayer->getPlayerSpeed() / 1.01 < 0.2)
                        gamePlayer->setPlayerSpeed(0.2);
        }
        ...
        while (gameWindow->pollEvent(event)) {
                ...
                if (event.type == sf::Event::KeyReleased and event.key.code == YELL) {
                        for (auto& obstacle : gameEntities) {
                                if (obstacle->getType() == "Door" and
                                    !obstacle
                                         ->isNoClip()) { // if door isnt opened a player can open it by pressing space
                                        std::pair<float, float> pPos = gamePlayer->getEPosition();
                                        std::pair<float, float> oPos = obstacle->getEPosition();
                                        if (oPos.second - pPos.second <= 0.6 and
                                            oPos.second - pPos.second >
                                                0) { // if distance between object and player is lower dan 0.6 and
                                                     // player and the object are in the same lane
                                                if (pPos.first > oPos.first - 0.25 and pPos.first < oPos.first + 0.25) {
                                                        obstacle->action(); // opens door
                                                        gamePlayer->notifyObservers(
                                                            DungeonRunner::Observer::staticObstacleAction); // penalizes
                                                                                                            // player
                                                        break;
                                                }
                                        }
...
void DungeonRunner::Game::spawnTraps(float dTime)
{
        ...
        for (auto& ai : aiPlayers) {
                if (ai->getEPosition().second > aiPos.second)
                        aiPos = ai->getEPosition();
        }
        if (aiPos.second > pPos.second)
                pPos = aiPos;
        if (Random::generateRandomChance() < 1.3 * dTime and
            pPos.second + 1 < 7) { // 1.3% chance every loop to spawn a new sword
...
void DungeonRunner::Game::run()
{
        ...
        while (gameWindow->isOpen()) {
                ...
                if (checkPos0 + 0.000001 < aiPlayers[0]->getAiSpeed() * dTime)
                        aiPlayers[0]->setIsStuck(true);
                else
                        aiPlayers[0]->setIsStuck(false);
                if (checkPos1 + 0.000001 < aiPlayers[1]->getAiSpeed() * dTime)
                        aiPlayers[1]->setIsStuck(true);
                else
                        aiPlayers[1]->setIsStuck(false);
                if (checkPos2 + 0.000001 < aiPlayers[2]->getAiSpeed() * dTime)
                        aiPlayers[2]->setIsStuck(true);
                else
                        aiPlayers[2]->setIsStuck(false);
        }
}
...
void DungeonRunner::Game::manageScores(float dTime)
{
        ...
        for (auto& entity : gameEntities) {
                if (entity->getType() == "Player" or entity->getType() == "AI") {
                        if (placement == 1)
                                entity->notifyObservers(DungeonRunner::Observer::firstPlace, dTime);
                        if (placement == 2)
                                entity->notifyObservers(DungeonRunner::Observer::secondPlace, dTime);
                        if (placement == 3)
                                entity->notifyObservers(DungeonRunner::Observer::thirdPlace, dTime);
                        if (placement == 4)
                                entity->notifyObservers(DungeonRunner::Observer::lastPlace, dTime);
                        placement--;
                }
        }
}
...
void DungeonRunner::Game::drawHighscores()
{
        ...
        endScore.emplace_back(gamePlayer->getSubjectObservers()[0]->getObserverName(),
                              gamePlayer->getSubjectObservers()[0]->getObserverData());
        for (auto& ai : aiPlayers) {
                endScore.emplace_back(ai->getSubjectObservers()[0]->getObserverName(),
                                      ai->getSubjectObservers()[0]->getObserverData());
        }
        std::sort(endScore.begin(), endScore.end(),
...
void DungeonRunner::Game::drawPlayerScore()
{
        ...
        playerScore.setPosition(gameView.getCenter().x - gameWindow->getSize().x / 2.0,
                                gameView.getCenter().y - gameWindow->getSize().y / 2.0);
